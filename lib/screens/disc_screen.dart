...
class DiscScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Disc')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.album, size: 100, color: Colors.purple),
            const SizedBox(height: 20),
            const Text('CD/DVD Player', style: TextStyle(fontSize: 24)),
            const SizedBox(height: 10),
            const Text('No disc inserted', style: TextStyle(color: Colors.grey)),
